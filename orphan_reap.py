#!/usr/bin/env python3
"""
orphan_reap.py  (Phase 2 - EXE duplicate-server fix & Process Tree Management)
==============================================================================
...
Provides WindowsJobObject for OS-level process group lifecycle management
(KILL_ON_JOB_CLOSE) and reap_process_tree.
"""
import subprocess
import sys
import time
...
# ------------------------------------------------------------------
# HARDENING: Windows Job Object + Process Tree Reap
# ------------------------------------------------------------------
...
def reap_process_tree(pid, timeout=3.0):
    """Recursively terminate a process and all descendants."""
    if not pid:
        return
    try:
        import psutil
        ...
        # Fallback using taskkill tree kill
        try:
            flags = subprocess.CREATE_NO_WINDOW if os.name == "nt" else 0
            subprocess.run(["taskkill", "/PID", str(pid), "/T", "/F"],
