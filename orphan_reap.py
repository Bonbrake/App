...
COMFY_PORT = 8188
SENTINEL = os.path.join(os.getenv("LOCALAPPDATA", r"C:\Users\jakeb\AppData\Local"),
...
def reap_orphan_8188(my_pid=None, dry_run=False):
    ...
    if sentinel_pid is None:
        print(f"[orphan_reap] no sentinel file - manual launch assumed, leave PID {pid}")
        return None
    if sentinel_pid == pid:
    ...
    if pid_alive(sentinel_pid):
        ...
        return None
    # sentinel process dead, and :8188 held by a DIFFERENT PID -> orphan from prior EXE
    if dry_run:
