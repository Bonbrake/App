...
# ── 6. CTk 6.0.0 thumbnail regression guard ─────────────────────────────
# In CTk 6.0.0, CTkLabel refuses to draw a raw PIL.ImageTk.PhotoImage — it
# shows only the fg_color background (black) and no image. Gallery + v2v
# thumbnails MUST wrap images in ctk.CTkImage. Grep the source to guarantee
# no reintroduction of the broken pattern in the thumbnail builders.
import re as _re
_src = open(SRC, encoding="utf-8", errors="replace").read()
_thumb_builders = _re.findall(
    r"def (_refresh_gallery_main|_refresh_gallery|_add_v2v_ref_thumbs|_show_thumb)\b",
    _src)
_bad_raw = False
_details = ""
for fn in set(_thumb_builders):
    m = _re.search(r"def %s\b" % fn, _src)
    body = _src[m.start():]
    nxt = body.find("\n    def ")
    if nxt > 0:
        body = body[:nxt]
    if "ImageTk.PhotoImage(" in body:
        _bad_raw = True
        _details = "%s uses raw ImageTk.PhotoImage" % fn
        break
chk("gallery: thumbnails use ctk.CTkImage (no raw PhotoImage)", not _bad_raw, _details)
